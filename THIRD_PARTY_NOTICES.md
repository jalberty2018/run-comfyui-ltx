...
The Dockerfile patches `ComfyUI-LTXVideo/pyramid_blending.py` to import `pad`
from `torch.nn.functional` and adjusts selected custom-node requirements.
These changes do not replace the upstream licenses. Preserve applicable notices
and provide corresponding modified source where the relevant license requires it.
The Dockerfile records the build-time changes; consult the installed revisions
and their license files for the full terms.
...
The [provisioning documentation](docs/ComfyUI_LTX_provisioning.md) and
[manual model downloads](docs/ComfyUI_LTX_provisioning.md) identify sources.
Each LTX video or audio model, text encoder, VAE, LoRA, segmentation model, upscaler and
