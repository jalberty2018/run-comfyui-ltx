...
The [model provisioning guides](ComfyUI_LTX_provisioning.md) cover LTX releases
and supporting audio, detection, segmentation, frame-interpolation and upscaling
models. Review each artifact's model card, license file and referenced original
publisher terms separately, including text encoders, VAEs, LoRAs and derivatives.
...
does not establish the license of the weights it downloads. For voice generation
or cloning, check both the model terms and the permissions required to use the
speaker's recordings and voice.
